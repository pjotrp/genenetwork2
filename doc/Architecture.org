...
Mainly the GnServer serves JSON and R/qtl2 data format requests, for
example to fetch data from the database. To get the menu data in YAML
...
currently in GN1 format for use in the main webserver, and will be
aligned with the [[http://kbroman.org/qtl2/pages/sampledata.html][R/qtl2 formats]] - as is currently served by the REST
API.
...
For the latest formats served see the [[#gnserver-rest][GnServer (REST)]] API.
